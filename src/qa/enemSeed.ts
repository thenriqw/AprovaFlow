...
export async function seedEnemQa(uid: string) {
  ...
  for (const s of DATA.subjects) {
    for (const t of s.topics) {
      ...
      else numActivities = randomInt(0, 1); // Reduces total activities to approx 100-125
  ...
  for (let i = 0; i <= totalDays; i++) {
    ...
    if (specialDays[dateStr]) {
      ...
      const r = rand();
      if (r < 0.15) dailySessions = 0; 
      else if (r < 0.40) dailySessions = 1; 
      else if (r < 0.75) dailySessions = 2; 
    ...
    for (const subName of targetSubjects) {
      ...
      let sessDur = 0;
      const rDur = rand();
      if (rDur < 0.15) sessDur = randomInt(15, 25) * 60; // Curtas
      else if (rDur < 0.75) sessDur = randomInt(45, 85) * 60; // Predominantes, avg 65
      else sessDur = randomInt(90, 120) * 60; // Longas
