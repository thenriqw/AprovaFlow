...
code = code.replace(/  const \[addingTopicTo, setAddingTopicTo\] = useState<string \| null>\(null\);\n  const \[newTopicName, setNewTopicName\] = useState\(''\);/, stateReplacement);
...
code = code.replace(/                      \{activities\.length > 0 \? \([\s\S]*?Nenhuma atividade registrada neste tópico\.<\/p>\n                      \)}/, renderReplacement);
